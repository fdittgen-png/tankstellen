  ...
  static Future<List<OpportunityCandidate>> detectRadiusAlerts({
    ...
    try {
      ...
      final fired = await runner.run(
        now: now,
        // #4185 — the dedup is stamped after the budget sends, not here.
        recordFire: false,
      ...
      final found = <OpportunityCandidate>[];
      final pendingByAlert = <String, RadiusAlertPendingFire>{
        for (final p in runner.pendingFires) p.alertId: p,
      };
      for (var i = 0; i < fired.length; i++) {
        final event = fired[i];
        if (event.matches.isEmpty) continue;
        final pending = pendingByAlert[event.alert.id];
