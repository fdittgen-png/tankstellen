...
///
/// ## Library location (#3131)
/// Neither platform bakes this function's library path into native config:
/// both fetchers pass it to `Workmanager().initialize(...)` at runtime,
/// which (re)persists the callback handle on every app launch
/// (`AppInitializer` → [BackgroundService.reconcile] → `init()`). The
/// native sides reference only task-name strings (`BootReceiver.kt`,
/// `BackgroundScanEnqueuer.kt`, `AppDelegate.swift` + Info.plist), which
/// are unchanged. Moving this file therefore only invalidates a handle
/// persisted by a *previous* install until the first post-update launch —
