...
// Re-export [stripPumpDisplayPollution] so tests and callers that
// already import it from this file keep resolving without changes.
export '_pump_display_pollution.dart' show stripPumpDisplayPollution;
...
class PumpDisplayParser {
  ...
  PumpDisplayParseResult parse(String rawText) {
    ...
    // Strip French pump-photo pollution (metrology header + card-reader
    // overlay + station stickers) before pattern matching so positional
    // inference sees only the three transaction numbers. The raw text
    // is still preserved for pump-number extraction.
    final stripped = stripPumpDisplayPollution(rawText);
    final normalised = normaliseDigits(stripped);
  ...
  ///
  /// Walks the RAW text (not the stripped text) so pollution stripping
  /// doesn't swallow the station-number sticker.
  int? _extractPumpNumber(String rawText, List<String> lines) {
    ...
    for (final line in rawLines) {
      if (kLonePumpDigitPattern.hasMatch(line)) {
