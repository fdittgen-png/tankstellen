...
void main() {
  ...
  group('fdroid lockfile regen (#3806)', () {
    ...
    test('the regen workflow is dispatch-only — never pull_request_target',
        () {
      final triggers = triggerBlock(read('lock-regen.yml'));
    ...
    test('build-fdroid names the fix when the lock is stale', () {
      ...
      expect(yaml, contains('Tools · Regenerate lockfiles'),
  ...
  // #3974 — `pubspec.lock` must be the resolution the Flutter SDK actually
  // produces. Dependabot maintains it with plain `dart pub`, which cannot
  // resolve `sdk: flutter` and so ignores the versions Flutter exact-pins
  // (`meta: 1.18.0`, `vector_math: 2.2.0`, `matcher: 0.12.19`). A plain
  // `flutter pub get` silently re-resolves instead of failing, so the drift
  // was invisible for as long as nothing enforced the file.
  group('pubspec.lock is the SDK\'s own resolution (#3974)', () {
    String read(String name) =>
        File('.github/workflows/$name').readAsStringSync();

    /// `package -> version` out of the **committed** lockfile at [path].
    ///
    /// Deliberately `git show HEAD:<path>` and not the working copy: the
    /// test runner does a `flutter pub get` before this file runs, which
    /// silently REWRITES a wrong `pubspec.lock` into a right one. Reading
    /// from disk therefore always passes — the exact false-green this test
    /// exists to catch. Only the committed bytes are evidence.
    Map<String, String> lockVersions(String path) {
      final show = Process.runSync('git', ['show', 'HEAD:$path']);
      expect(show.exitCode, 0,
          reason: 'could not read committed $path: ${show.stderr}');
      final versions = <String, String>{};
      final package = RegExp(r'^  ([A-Za-z0-9_]+):\s*$');
      final version = RegExp(r'^    version: "?([^"\n]+)"?\s*$');
      String? current;
      for (final line in const LineSplitter().convert('${show.stdout}')) {
        final p = package.firstMatch(line);
        if (p != null) {
          current = p.group(1);
          continue;
        }
        final v = version.firstMatch(line);
    ...
    // The packages the Flutter SDK exact-pins through flutter/flutter_test.
    // Both lockfiles are resolved by the SAME SDK — the fdroid overrides only
    // swap the GMS/ML-Kit/Play-Core/Sentry plugins — so these must agree.
    // On master before this fix they disagreed on every single one, which is
    // exactly what gave the bug away.
    const sdkPinned = [
      'meta',
      'vector_math',
      'matcher',
    ...
    test('both lockfiles agree on every SDK-pinned package', () {
      final main = lockVersions('pubspec.lock');
      final fdroid = lockVersions('pubspec.fdroid.lock');
      final disagreements = <String>[];
      for (final p in sdkPinned) {
        final a = main[p];
        final b = fdroid[p];
        if (a == null || b == null) continue;
        if (a != b) disagreements.add('$p: pubspec.lock=$a fdroid=$b');
      }
      expect(disagreements, isEmpty,
          reason: 'pubspec.fdroid.lock is generated by a real '
              '`flutter pub get`, so where the two disagree on an '
              'SDK-pinned package it is pubspec.lock that is wrong — '
              'regenerate it through Flutter, not `dart pub` (#3974). '
              'Disagreements:\n${disagreements.join('\n')}');
    });

    test('analyze enforces the lockfile rather than silently re-resolving',
        () {
      expect(read('ci.yml'), contains('flutter pub get --enforce-lockfile'),
          reason: 'a plain `flutter pub get` rewrites the lock instead of '
              'failing, so the drift stays invisible — the main flavour '
              'needs the same gate build-fdroid always had');
    });

    test('the regen workflow owns BOTH lockfiles', () {
      final regen = read('lock-regen.yml');
      expect(regen, contains('pubspec.fdroid.lock'));
      expect(regen, contains('Regenerate pubspec.lock'),
          reason: 'regenerating only the fdroid lock leaves the main lock '
              'to Dependabot, which is what caused #3974');
    });
  });
